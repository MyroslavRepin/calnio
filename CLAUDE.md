...
- Sync runs **Notion → Apple Calendar** always, and **Apple Calendar → Notion per sync**, off by default (`sync_mappings.write_back`). Two-way covers the full round: dates, titles, new events becoming pages, deleted events trashing pages.
- Writing needs a Notion grant consented to with update + insert capabilities. `notion_connections.can_write` records it, and it is **False for every grant made before two-way shipped**: those users reconnect or stay one-way. A write refused with `restricted_resource` flips the flag back to False.
...
- `repo/` — data access, filenames never repeat the folder: `caldav.py` (`CalDavAccountRepo` for calendars, `CalDavEventRepo` for events in one calendar plus `changes()`, the RFC 6578 sync-collection read), `notion.py` (`NotionPageRepo`: reads, plus `create_page`/`update_page`/`trash_page`, which are the only writes Calnio ever makes into a workspace), `user.py` (`UserRepo(db: Session)`, `get_or_create_user_oauth` = login and registration in one), `notion_connection.py` (grant rows plus `revoke()`), `caldav_credential.py`, `sync_settings.py`, `sync_mapping.py` (`SyncMappingRepo`, every read scoped by `user_id` so another account's id reads as missing), `admin.py` (`AdminRepo`, aggregates across every account for the admin dashboard, counts only and never a token, a password or a page title). A repo's client is built in `__init__`, so constructing one authenticates. **Caller owns the transaction and the commit**, except `services/sync.py`, which commits per event deliberately.
...
- `api/` — route handlers, one literal path per decorator: `oauth.py` (`/auth/*`, since Google's registered redirect URI depends on it), `apple_calendar.py` (`/api/v1/me/apple-calendar*`, per-user iCloud credentials), `notion.py` (`/auth/oauth/notion/*` + `/api/v1/me/notion*`), `sync.py` (`/api/v1/me/sync`, the per-user master switch), `mapping.py` (`/api/v1/me/syncs*`, the N syncs: list, create, configure, delete, per-sync date-properties), `account.py` (`DELETE /api/v1/me`, a hard delete: the `users` row goes, children cascade, the Notion grant is revoked, a queued one-off sync job is cancelled, auth cookies are cleared; **iCloud is deliberately untouched**, pushed events are the user's data and stay, same as an Apple Calendar disconnect. Body must echo the signed-in email). `admin.py` (`GET /api/v1/admin/stats`, one answer holding every number the admin page draws). New API routers take the `/api/v1` prefix so they don't collide with the SPA served at `/` in prod.
- `deps/` — FastAPI dependencies and the plumbing routers share: `db.py` (`get_session`), `auth.py` (`get_current_user` reads the access cookie and returns the `User` row, `get_admin_user` refuses a non-admin with **404** rather than 403 so an ordinary account learns nothing, plus `set_auth_cookies` / `clear_auth_cookies`), `notion.py` (`get_connection`, `get_notion_repo`, `notion_errors`), `caldav.py` (`get_credential`, `icloud_errors`, `icloud_credentials`), `sync.py` (`get_sync_settings`, `has_eligible_mapping`, `sync_status`, `queue_run`), `mapping.py` (`get_mapping` scoped to the signed-in user, `date_property_names`, `pick_date_property`, `calendar_name_for`, `mapping_eligible`, `mapping_status`). `queue_run` lives in `deps/` rather than `api/sync.py` because two routers queue a run, and routers never import from routers.

### Sync model (core of the app)

- Source of truth: Notion, and it wins every tie. Mapping key: Notion page id == iCal `uid`, **except** for an event the user made in Apple Calendar, which keeps Apple's uid: that is why `synced_events` has both `notion_page_id` and `caldav_uid`.
- `synced_events` is the **merge baseline**, not just a link index: `title` + `start_at` + `end_at` + `all_day` hold what the two sides last agreed on. Notion differing from it means Notion was edited, the calendar differing from it means the calendar was, both differing is the only real conflict. Comparing content rather than timestamps is also what stops the two directions echoing: Calnio rewrites the baseline on every push, so its own write is never read back as a user's edit. `start_at IS NULL` marks a row from before two-way: Notion rebuilds it, no calendar edit is inferred from it. Only rows Calnio owns; foreign Apple Calendar events (no row) are never touched.
...
- `services/sync.py` per mapping: query the data source → keep live pages (not archived, with a date in that mapping's column) → when two-way is on, ask the calendar what changed → `merge_pages` reconciles every live page with its event → `drop_pages` deletes events whose page is gone → `import_events` turns unknown calendar events into pages. **Commits per event on purpose** — a failure mid-batch must not orphan CalDAV events (uncommitted row → next run re-creates → iCloud 412 duplicate).
- **Deletions are heard only through the sync token.** A deleted CalDAV resource leaves nothing behind, not even a time of death, so `CalDavEventRepo.changes(token, known_hrefs)` does an RFC 6578 sync-collection report: iCloud supports it and names removed resources. No token, or a token iCloud forgot, falls back to a full listing, and a deletion is then an href the caller knows and the server did not list. Hrefs are matched by path (`href_path`), never whole.
- The token stored after a run is the one fetched **before** that run's own writes. The next run sees Calnio's writes again, finds them equal to the baseline and does nothing. Repeating a read is cheap, missing a change is not.
- **Conflict rule:** both sides changed since the last run → last write wins, and Notion takes every tie (`notion_wins`). Notion's `last_edited_time` is rounded down to the minute, so the event's `LAST-MODIFIED` is truncated to the minute before comparing. An event deleted in Apple while its page was edited comes back: a delete carries no timestamp, so it cannot win a race it cannot enter.
- **Import guards** (calendar event with no link row → new Notion page): only when two-way is on, only when the calendar has exactly **one** mapping (`may_import`, since a shared calendar cannot say which database the event belongs in), only for events created after `write_back_since` (so switching two-way on does not import a calendar's existing events), and never for a repeating event or an invite with attendees.
- Updates to CalDAV edit the existing VEVENT in place (`ICalParser.apply_event`) rather than replacing it, because an event the user made carries alarms, notes and a location Calnio has no column for.
- **Grain:** `sync_mapping(db, mapping, notion, credential, can_write=...)` runs one mapping and returns its status. `sync_user(user_id)` loads the grant, the credential and the master switch once, builds **one** `NotionPageRepo` for the user, then loops their eligible mappings. `run_all_users()` is the scheduled job and syncs every eligible user sequentially. The scheduling grain is still the user: job id `sync-user-{id}`.
...
- Change detection is content against the baseline (`same_event`), not timestamps. Timestamps only break a tie.
- A write refused by Notion with `restricted_resource` raises `NotionReadOnly`: the grant's `can_write` goes False, two-way stops for that user, and the forward sync keeps running. It is deliberately **not** an auth failure, which would disable the master switch.
...
- **The run id is stored, not just logged.** A failed run writes `sync_mappings.last_error` and `last_run_id`, so the sync card tells the user what broke and the admin page hands over `grep "run=<id>" /srv/calnio/logs/calnio.log`. A successful run clears both.
...
**Admin page.** `/dashboard/admin` renders `AdminView`, and the sidebar links to it only when `state.user.is_admin`. Hiding the link is tidiness: the API answers 404 to everybody else, which is the actual protection.
